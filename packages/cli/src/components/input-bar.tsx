...
function isMentionQueryChracter(chracter: string) {
  return MENTION_QUERY_CHARACTER.test(chracter);
}
...
function findActiveMention(
  ...
  if (previousChracter && isMentionQueryChracter(previousChracter)) {
    return null;
  }
  ...
  while (
    ...
    isMentionQueryChracter(token[mentionEnd]!)
  ) {
    mentionEnd += 1;
  }
  ...
  try {
    ...
    const visit = async (
      ...
      for (const entry of entries) {
        ...
        const path = currentDirectoryPart
          ? `${currentDirectoryPart}/${entry.name}`
          : entry.name;
        const kind: MentionCandidate["kind"] = entry.isDirectory()
          ? "directory"
          : "file";
        ...
        if (entry.isDirectory()) {
          await visit(resolve(currentDirectory, entry.name), path);
          if (fallbackMatches.length >= MAX_FALLBACK_MENTION_CANDIDATES) {
            return;
          }
        }
...
export function InputBar({ onSubmit, disabled = false }: Props) {
  ...
  const handleMentionExccute = useCallback(
    (index: number) => {
      const textarea = textareaRef.current;
  ...
  useEffect(() => {
    onSubmitRef.current = () => {
      ...
      if (showMentionMenu) {
        ...
        if (candidate) {
          handleMentionExccute(mentionSelectedIndex);
          return;
        }
      }
      handleSubmit();
          ...
          {!showCommandMenu && showMentionMenu && (
            ...
            >
              <FileMentionMenu
                ...
                onExecute={handleMentionExccute}
              />
            </box>
          )}
          <textarea
