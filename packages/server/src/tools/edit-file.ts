...
export function createEditFileTool(cwd: string) {
  return tool({
    description: `Make a targeted edit to a file by replacing an exact string match. the oldString
    must appear exactly once in the file (for safety), Use this for surgical edits instead of
    ...
    execute: async ({ path, oldString, newString }) => {
      ...
      try {
        ...
        if (occurrences > 1) {
          return {
            error: `oldString is ambiguous - found ${occurrences} matches. Provide
